...
/*static void deployCubes(void){
  ...
  static void armFunc(){
       Arm.setVelocity(Controller1.Axis2.position(), percent);
       Arm.spin(fwd);
  }
...
Arm.spin(reverse);
wait(200, msec);
int prevpos = Controller1.Axis2.position();
  while (1) {
    ...
    if(Controller1.Axis2.position()!=prevpos)
    {
      armFunc();
    }

    prevpos = Controller1.Axis2.position();
